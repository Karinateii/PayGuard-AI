    ...
    private async Task SimulateTransaction()
    {
        ...
        try
        {
            ...
            var request = new
            {
                ...
                senderId = string.IsNullOrWhiteSpace(_senderId) ? (string?)null : _senderId
            };
            var response = await client.PostAsJsonAsync(baseUrl + "/api/webhooks/simulate", request);
            if (response.IsSuccessStatusCode)
            {
                var json = await response.Content.ReadFromJsonAsync<SimApiResponse>();
                if (json != null && json.Success)
                {
                    _lastResult = new SimResult
                    {
                        TransactionId = json.TransactionId ?? "",
                        Amount = json.Amount ?? _amount,
                        SourceCountry = json.SourceCountry ?? _sourceCountry,
                        DestinationCountry = json.DestinationCountry ?? _destinationCountry,
                ...
                else { Snackbar.Add("Simulation failed", Severity.Error); }
            }
