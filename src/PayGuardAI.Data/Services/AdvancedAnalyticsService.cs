...
public class AdvancedAnalyticsService : IAdvancedAnalyticsService
{
    ...
    {
        // Placeholder: Would integrate with a PDF library (e.g., QuestPDF, PdfSharp)
        // For now, return a simple text-based "PDF" (really just text)
        var sb = new StringBuilder();
    ...
    public async Task<List<AnalystPerformance>> GetAnalystPerformanceAsync(string tenantId, int days = 30, CancellationToken ct = default)
    {
        ...
        var analystGroups = auditLogs
            ...
            .Select(g =>
            {
                ...
                var reviewTimes = g.Select(a => ExtractReviewTime(a.Notes)).Where(t => t > 0).ToList();
    ...
    public async Task<List<ReviewTimeDistribution>> GetReviewTimeDistributionAsync(string tenantId, int days = 30, CancellationToken ct = default)
    {
        ...
        var reviewTimes = await _db.AuditLogs
            .Where(a => a.TenantId == tenantId 
                     && a.CreatedAt >= cutoff
                     && (a.Action == "TRANSACTION_APPROVED" || a.Action == "TRANSACTION_REJECTED"))
            .Select(a => ExtractReviewTime(a.Notes))
            .Where(t => t > 0)
            .ToListAsync(ct);
    ...
    public async Task<DecisionConsistencyMetrics> GetDecisionConsistencyAsync(string tenantId, int days = 30, CancellationToken ct = default)
    {
        ...
        var decisions = await _db.AuditLogs
            .Where(a => a.TenantId == tenantId 
                     && a.CreatedAt >= cutoff
                     && (a.Action == "TRANSACTION_APPROVED" || a.Action == "TRANSACTION_REJECTED"))
            .Join(_db.Transactions, a => Guid.Parse(a.EntityId ?? ""), t => t.Id, (a, t) => new { a, t })
            .Join(_db.RiskAnalyses, x => x.t.Id, r => r.TransactionId, (x, r) => new { x.a, x.t, r })
            .ToListAsync(ct);
        ...
        {
            var band = GetRiskBand(d.r.RiskScore);
            riskBands[band].TotalDecisions++;
            if (d.a.Action == "TRANSACTION_APPROVED")
        ...
        return new DecisionConsistencyMetrics
        {
            ...
            OverallConsistency = 0.85, // Placeholder — would calculate variance across analysts
    ...
    public async Task<RoiMetrics> CalculateRoiAsync(string tenantId, DateTime? startDate = null, CancellationToken ct = default)
    {
        ...
        var reviewedTransactions = await _db.AuditLogs
            .Where(a => a.TenantId == tenantId 
                     && a.CreatedAt >= onboardingDate
                     && (a.Action == "TRANSACTION_APPROVED" || a.Action == "TRANSACTION_REJECTED"))
            .ToListAsync(ct);

        var rejections = reviewedTransactions.Count(a => a.Action == "TRANSACTION_REJECTED");
        var approvals = reviewedTransactions.Count(a => a.Action == "TRANSACTION_APPROVED");

        // Estimate fraud value prevented (assume rejections were fraud)
        var fraudValuePrevented = allTransactions
            .Where(t => reviewedTransactions.Any(r => Guid.Parse(r.EntityId ?? "") == t.Id && r.Action == "TRANSACTION_REJECTED"))
            .Sum(t => t.Amount);

        // False positive rate over time
        var firstMonth = allTransactions.Where(t => t.CreatedAt < onboardingDate.AddDays(30));
        var recentMonth = allTransactions.Where(t => t.CreatedAt >= DateTime.UtcNow.AddDays(-30));

        var firstMonthFPRate = CalculateFalsePositiveRate(firstMonth, reviewedTransactions.Where(r => r.CreatedAt < onboardingDate.AddDays(30)));
        var recentMonthFPRate = CalculateFalsePositiveRate(recentMonth, reviewedTransactions.Where(r => r.CreatedAt >= DateTime.UtcNow.AddDays(-30)));

        var fpReduction = firstMonthFPRate > 0 ? ((firstMonthFPRate - recentMonthFPRate) / firstMonthFPRate) * 100 : 0;
        ...
        return new RoiMetrics
        {
            ...
            FalsePositives = approvals, // Approximation
            TruePositives = rejections,
            Precision = (rejections + approvals) > 0 ? (double)rejections / (rejections + approvals) : 0,
            Recall = 0, // Would need false negatives (fraud that wasn't caught)
            FalsePositiveRate = recentMonthFPRate,
            FalsePositiveRateAtOnboarding = firstMonthFPRate,
            FalsePositiveReduction = fpReduction,
            CostSavings = fraudValuePrevented - (reviewedTransactions.Count * 5), // Assume $5 per review
    ...
    private double ExtractReviewTime(string? notes)
    {
        // Placeholder: Parse "Reviewed in X minutes" from notes
        // In production, track review start/end times properly
        return 0;
    }
    ...
    private double CalculateFalsePositiveRate(IEnumerable<Transaction> transactions, IEnumerable<AuditLog> reviews)
    {
        var flagged = transactions.Count();
        if (flagged == 0) return 0;

        var falsePositives = reviews.Count(r => r.Action == "TRANSACTION_APPROVED");
        return (double)falsePositives / flagged * 100;
    }
